...
void Section::writeSectionFileHeader(const ReaderRenderSpec& spec) {
  ...
  static_assert(HEADER_SIZE == sizeof(SECTION_FILE_VERSION) + sizeof(spec.fontId) + sizeof(spec.lineCompression) +
                                   ...
                                   sizeof(spec.hyphenationEnabled) + sizeof(spec.firstLineIndent) +
...
bool Section::loadSectionFile(const ReaderRenderSpec& spec) {
  ...
  bool filePartial = false;
  {
    ...
    if (spec.fontId != fileFontId || spec.lineCompression != fileLineCompression ||
        ...
        spec.hyphenationEnabled != fileHyphenationEnabled || spec.firstLineIndent != fileFirstLineIndent ||
...
Section::StartBuildResult Section::startBuild(const ReaderRenderSpec& spec, const std::function<void()>& popupFn,
  ...
  ctx->parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
      ...
      spec.paragraphAlignment, spec.viewportWidth, spec.viewportHeight, spec.hyphenationEnabled, spec.firstLineIndent,
